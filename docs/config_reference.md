...
## Optical-Flow Reliability Configuration: `RELIABILITY_CFG`

| Name | Meaning |
| --- | --- |
| `ENABLE_FB_CONSISTENCY` | Enables forward-backward optical-flow consistency diagnosis. |
| `FB_ERROR_THRESHOLD` | Maximum forward-backward error considered reliable. |
| `SAVE_FB_DEBUG_IMAGES` | Saves reliability masks and error visualizations. |
| `APPLY_RELIABLE_MASK_TO_DETECTION` | Applies the reliability mask to detection instead of using it only for diagnosis. |

## Appearance-Change Configuration: `APPEARANCE_CFG`

| Name | Meaning |
| --- | --- |
| `ENABLE_APPEARANCE_CHANGE` | Enables the Version 2.0 appearance-change tool. It is disabled in the backed-up default configuration. |
| `FUSION_MODE` | Combines flow and appearance masks using `flow_only`, `appearance_only`, `flow_and_appearance`, or `flow_or_appearance`. |
| `ENABLE_COLOR_CHANGE` | Detects HSV color changes between consecutive frames. |
| `COLOR_DIFF_THRESHOLD` | Minimum color difference marked as changed. |
