    ...
    try:
        ...
        while True:
            ...
            if APPEARANCE_CFG["ENABLE_APPEARANCE_CHANGE"]:
                ...
                flow_pre_fusion_pack = postprocess_target_mask(mask_used, POST_CFG)
                mask_used = fuse_flow_and_appearance_masks(
                    flow_pre_fusion_pack["post_mask_bool"],
                    appearance_pack["appearance_mask"],
                    APPEARANCE_CFG,
                )
                fused_target_pixels = int(mask_used.sum())
